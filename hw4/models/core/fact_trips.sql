...
select
    ...
    case
        when extract(month from trips_unioned.pickup_datetime) in (1, 2, 3)
        then 1
        when extract(month from trips_unioned.pickup_datetime) in (4, 5, 6)
        then 2
        when extract(month from trips_unioned.pickup_datetime) in (7, 8, 9)
        then 3
        when extract(month from trips_unioned.pickup_datetime) in (10, 11, 12)
        then 4
